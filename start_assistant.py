...
def start_codeassistant() -> Tuple[bool, Optional[int]]:
    ...
    try:
        ...
        codeassistant_process = subprocess.Popen(
            cmd, env=env, stdout=open(api_log, "a"), stderr=subprocess.STDOUT
        )

        # 记录启动的进程
        pid = codeassistant_process.pid
        processes["codeassistant"] = {
            "process": codeassistant_process,
            "pid": pid,
            "log": str(api_log),
        }
        ...
        for i in range(max_retries):
            ...
            if check_api_health(host, port):
                startup_logger.info(
                    f"CodeAssistant服务启动成功，API地址: http://{host}:{port}"
                )
                ...
                if HAS_VISUALIZATION and visualization_manager:
                    try:
                        ...
                        dashboard_path = visualization_manager.dashboard.visualize(
                            "system_monitor", dashboard_data, title="AIgo系统启动监控"
                        )
                        ...
                        if HAS_FEATURE_INTEGRATION and feature_integrator:
                            try:
                                ...
                                integration_results = (
                                    feature_integrator.integrate_features()
                                )

                                # 记录集成结果
                                integrated_count = sum(
                                    1 for v in integration_results.values() if v
                                )
                                startup_logger.info(
                                    f"共集成了 {integrated_count}/{len(integration_results)} 个高级特性"
                                )

                                # 创建功能结构图
                                if integrated_count > 0 and hasattr(
                                    visualization_manager.structure, "visualize"
                                ):
                                    # 构建特性数据
                                    features_data = []
                                    for (
                                        ...
                                        if success:
                                            feature_info = (
                                                feature_integrator.enabled_features.get(
                                                    feature_id, {}
                                                )
                                            )
                                    ...
                                    if integration_results.get("model_optimization"):
                                        optimizer = (
                                            feature_integrator.get_feature_integrator(
                                                "model_optimization"
                                            )
                                        )
                                        if optimizer:
            ...
            try:
                ...
                if "codeassistant" in processes:
                    ...
                    if not api_running and not stop_event.is_set():
                        startup_logger.warning(
                            f"CodeAssistant进程(PID: {pid})已终止，尝试重启"
                        )
                ...
                if HAS_VISUALIZATION and visualization_manager:
                    try:
                            ...
                            model_info.append(
                                {
                                    ...
                                    "parameters": (
                                        model.get("parameter_size", 0) // (1024 * 1024)
                        ...
                        if model_info:
                            ...
                            art_data = {"files": model_info}
                            art_path = visualization_manager.art_generator.visualize(
                                "code_galaxy", art_data, title="AIgo模型星系"
                            )
                            ...
                            if HAS_FEATURE_INTEGRATION and feature_integrator:
                                if (
                                    "model_optimization"
                                    in feature_integrator.enabled_features
                                ):
                                    try:
                                        optimizer = (
                                            feature_integrator.get_feature_integrator(
...
def main():
    ...
    parser.add_argument(
        "--no-ollama", action="store_true", help="不启动Ollama服务(假设已经运行)"
    )
    ...
    parser.add_argument(
        "--enable-features", type=str, help="启用指定的高级特性，逗号分隔"
    )
    ...
    if not check_ollama_installed():
        startup_logger.error(
            "未检测到Ollama安装，请先安装Ollama: https://ollama.com/download"
        )
        return 1
    ...
    if args.visualize and not HAS_VISUALIZATION:
        startup_logger.warning(
            "请求启用可视化功能，但缺少必要的依赖库。请先安装以下库: matplotlib, plotly, networkx, graphviz"
        )
    ...
    if not args.no_browser:
        ...
        if (
            HAS_VISUALIZATION
            and HAS_FEATURE_INTEGRATION
            and "FEATURE_MAP_PATH" in os.environ
        ):
